...
| Management/endpoint allowlist | P0 state/value/endpoint/count checks now run at `rtkit_akf_send`, covering the operations-table calls from boot, endpoint start, buffer reply and power switching. Exact command-buffer/tag registrations and one outstanding ring are checked. Synthetic v10-v12 policy and actual-wrapper rejection tests pass; see `rtkit-allowlist.md`. **Remaining:** generic proxy/session exclusion and separate SEP raw-AKF callers; complete inbound-field and live-trace verification. Do not interpret this as a complete controlled-host/MMIO audit. |
| Timeout and budgets | Boot/power waits and mailbox sends have timeouts; RX batches are limited to 64. P0 AKF wrappers now cap TX at 128 attempts and RX at 512 messages per RTKit instance, including timed HELLO/EPMAP RX. Exhaustion or TX failure latches the RTKit error state, preventing subsequent TX; syslog/ioreport ACK failure returns an error. `test_rtkit_budget.py` exercises actual wrappers at the limits, send failure and empty timed RX. One-attempt ANS initialization prevents resetting counters through controller restart. Crash record parsing is skipped for P0, retaining raw data. **Remaining:** complete the state/value allowlist and reject unexpected inbound management/app messages; transport count limits do not prove protocol validity. |
| MMIO and firmware mapping | **Incomplete.** Before power enable, `akf_p0_validate_ans` now requires the J42d inventory's compatible and four exact base/size pairs; it is also checked before AKF initialization. Synthetic ADT tests reject each wrong base/size, failed lookup and wrong compatible. The firmware range parser rejects overflow, malformed/overlarge lists, holes and inconsistent phys/IOVA translations; malformed new metadata cannot fall back to old fields. Still require permitted MMIO offsets/write counts, PMGR parent-domain audit and live heap non-overlap. This metadata gate does not attest the individual ECID. |
...
| Next-stage exclusion | The P0 buffer preparation seals proxy dispatch before allocation. Only NOP and limited ANS operations remain; raw writes are denied and raw reads are restricted to the bound buffer. Failed operations abort without unsealing. Predicate/transfer-bound tests and build pass; see `session-lock.md`. Generic free/call/boot/SEP entry points are denied after sealing. **Remaining:** full transport/exception-path and current-boot integration verification, not a blanket proof against every possible reset. |
