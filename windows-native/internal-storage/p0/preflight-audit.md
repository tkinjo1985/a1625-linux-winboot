...
| MMIO and firmware mapping | **Incomplete.** `akf_init` accepts ADT register base, and `akf_map_preloaded_fw` writes the returned region directly. The range parser now rejects overflow, malformed/overlarge lists, holes and inconsistent phys/IOVA translations; malformed new metadata cannot fall back to old fields. Actual-source synthetic ADT tests pass. Still require exact current-device ADT bounds, permitted MMIO offsets/write counts and heap non-overlap before these writes. |
